...
[TestFixture]
public class PostSignedAdminOperationHandlerTests
{
    ...
    public async Task Handle_Publishes_Event_Excluding_Self()
    {
        ...
        _publisher
            .Setup(p => p.Publish(It.IsAny<SignedAdminOperationPostedEvent>(), It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);

        var handler = new PostSignedAdminOperationHandler(_resolver.Object, _publisher.Object, _selfIdentity.Object);
        var cmd = new PostSignedAdminOperationCommand(
            lookup,
            opId,
            sentUtc,
            Percolator.Chat.Events.AdminOperationKind.GrantAdmin,
            new byte[] { 1, 2, 3 },
            null,
            null,
            null,
        ...
        // Assert
        _resolver.VerifyAll();
        _publisher.Verify(p => p.Publish(It.IsAny<SignedAdminOperationPostedEvent>(), It.IsAny<CancellationToken>()), Times.Once);
    }
}
