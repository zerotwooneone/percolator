using Percolator.Network;
using Percolator.Network.Messaging;

namespace Percolator.Application.Network.Messaging;

public interface IRouteSender
{
    // Attempt direct delivery to target. Implementation decides endpoint selection.
    Task<TransportSendResult> SendDirectAsync(NetworkPeerId target, NetworkPayload payload, CancellationToken ct = default);

    // Attempt relay delivery using the relay peer to the final target. Implementation handles envelope construction.
    Task<TransportSendResult> SendViaRelayAsync(NetworkPeerId relay, NetworkPeerId target, NetworkPayload payload, CancellationToken ct = default);
}
