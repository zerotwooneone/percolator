...
public sealed class SecureMessagingService : ISecureMessagingService
{
    ...
    public async Task<(SessionId sessionId, Plaintext plaintext)?> DecryptInboundAsync(CryptoSelfId selfIdentityId, SessionRatchetMessage message, CancellationToken cancellationToken = default)
    {
