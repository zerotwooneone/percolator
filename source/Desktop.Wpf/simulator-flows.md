...
- **Relay Node**: A Simulated Peer explicitly flagged as a router. It intercepts, queues, and forwards opaque payloads.
...
4. **Sessions**: Cryptographic state inspection (Direct Ratchets & Group Epochs).
...
### Flow 11 — Group Chat & Sender Keys (Future/Concept)
**Goal:** Validate multicast E2EE environments.
1. Main Node establishes a Group Chat involving Simulator Peers A and B.
2. Simulator **Sessions** tab displays a `GROUP` session card.
3. User observes the **Sender Key Epoch**. When a peer is added or removed, the Epoch number increments, reflecting successful Key Rotation.

### Flow 12 — State Persistence & Reset
**Goal:** Ensure continuity across development sessions.
1. Simulator state (Peers, Queues, Ratchet Indexes) is persisted to local storage.
2. Upon browser refresh/app restart, the simulator reloads the exact state. Active Double Ratchets remain perfectly in sync with the Main Node.
