...
## chunk K: Plan — Fix relayed standard handshake OTK mismatch (simulator responder)

### K.0: Problem statement (symptom)

After a relayed standard handshake (main -> simulator relay -> simulated target) appears to complete, the first encrypted chat message fails to decrypt on the simulated target with `AuthenticationTagMismatchException`.

Observed debugging signal:

- Main encrypts with a session whose ratchet state increments as expected.
- Simulated target attempts to decrypt with the “same” session id, but cannot authenticate/decrypt, which is consistent with the two sides deriving different initial session keys (different X3DH shared secret / different root key).

### K.1: Leading theory (root cause)

The simulator’s standard-handshake responder path ignores the one-time prekey (OTK) selected by Main. When Main initiates X3DH using an OTK from the target’s published bundle, the simulated target always derives X3DH without that OTK, producing a different shared secret.

Evidence anchor:

- `Desktop.Wpf/Features/Simulator/SimulatorStateService.cs` → `ReceiveEstablishSessionFromMainAsync(...)`
    - Calls `crypto.X3DH_Respond(... localOtkPrivate: null)` unconditionally.
    - Does not read `request.OnetimePrekeyId`.
- `Desktop.Wpf/Features/Simulator/Protocol/SignalProtocolEngine.cs` → `CreateStandardPreKeyBundle(...)`
    - Generates OTKs but stores only the public key bytes (`ExportSubjectPublicKeyInfo()`), and the simulator does not retain private key material for later responder use.

### K.2: Design constraints (what must be true after the fix)

- If Main includes `HandshakeInitiatorHello.OneTimePreKeyId`, the simulated target must:
    - Locate the matching OTK private key by id.
    - Use it in `X3DH_Respond(... localOtkPrivate: ...)`.
    - Consume it at most once (remove from available set).
- The relay host’s published-bundle store remains **public-only** (it should not contain private key material).
- Snapshot persistence must retain the simulated target’s unconsumed OTK private keys so that restarting the simulator does not break pending handshakes.

### K.3: Implementation-ready design (strong types + exact behaviors)

This fix must eliminate all ambiguity: the simulated responder must use the *same* OTK (by id) that the initiator used.

#### K.3.1: Introduce strong types for OTK ids and private records (Desktop.Wpf)

**Why:** today OTK ids move around as raw `Guid` and bytes; for correctness we want a single canonical “OTK id” type and a single canonical “private OTK record” type.

**File:** `Desktop.Wpf/Features/Simulator/Models/SimulatedOneTimePreKeyId.cs` (new)
...
**File:** `Desktop.Wpf/Features/Simulator/Models/SimulatedOneTimePreKeyPrivateRecord.cs` (new)

- Add:
  - `public sealed record SimulatedOneTimePreKeyPrivateRecord(
        SimulatedOneTimePreKeyId Id,
        Percolator.Cryptography.PrivatePreKey PrivateKey,
        DateTimeOffset CreatedAtUtc);
`

Notes:

- Use `Percolator.Cryptography.PrivatePreKey` as the in-memory representation (strong type) instead of `byte[]`.
- Persist `PrivateKey.Value` (bytes) only at the snapshot boundary.

#### K.3.2: Store private OTKs on the logical owner (`SimulatedPeerModel`) with pop semantics

**File:** `Desktop.Wpf/Features/Simulator/SimulatedPeerModel.cs`

- Add a new collection:
  - `internal ObservableList<SimulatedOneTimePreKeyPrivateRecord> OneTimePreKeysPrivateMutable { get; }`
  - `public IReadOnlyObservableList<SimulatedOneTimePreKeyPrivateRecord> OneTimePreKeysPrivate { get; }`

- Add a single authoritative pop method (no “search/remove” duplicated at call sites):

  - `internal bool TryPopOneTimePreKeyPrivate(SimulatedOneTimePreKeyId id, out Percolator.Cryptography.PrivatePreKey privateKey)`

**Required behavior (non-negotiable):**

- If found:
  - Remove the record from `OneTimePreKeysPrivateMutable` *before returning*.
  - Return `true` and set `privateKey`.

- If not found:
  - Return `false` and set `privateKey = default`.

**No other code** should remove OTK private records directly; all consumption must go through this method.

#### K.3.3: Generate OTK keypairs and publish only the public portion

**File:** `Desktop.Wpf/Features/Simulator/Protocol/SignalProtocolEngine.cs`

**Method:** `CreateStandardPreKeyBundle(SimulatedPeerModel peer, DateTimeOffset? expiresUtc, int oneTimeKeyCount)`

Update the OTK generation loop so each generated OTK produces:

- **Public publication output** (unchanged externally):
  - `Percolator.Cryptography.OneTimeKeyInstance` where:
    - `Id` matches the generated id.
    - `KeyBytes` contains the public SPKI bytes as it does today.

- **Private storage** (new):
  - Add `SimulatedOneTimePreKeyPrivateRecord` to `peer.OneTimePreKeysPrivateMutable` containing:
    - the same id
    - the private key as `PrivatePreKey`
    - a `CreatedAtUtc`

**Required invariants:**

- The relay host’s `PublishedPreKeyBundles` remain **public-only**.
- There must be a 1:1 mapping between each `OneTimeKeyInstance` published and a private record stored on the peer at creation time.
- No OTK private key material is ever stored in `SimulatedPublishedPreKeyBundleModel`.

#### K.3.4: Responder MUST use `request.OnetimePrekeyId` when present

**File:** `Desktop.Wpf/Features/Simulator/SimulatorStateService.cs`

**Method:** `ReceiveEstablishSessionFromMainAsync(PeerId simulatedPeerId, EstablishSessionRequest request, CancellationToken ct)`

Add logic:

- Determine whether an OTK was requested:
  - OTK is requested iff:
    - `request.OnetimePrekeyId` exists AND
    - it contains a valid `Guid` (exact parsing logic below)

- Parsing rule (explicit):
  - `var otkGuid = new Guid(request.OnetimePrekeyId.ToByteArray());`
  - `var otkId = new SimulatedOneTimePreKeyId(otkGuid);`

- If OTK requested:
  - Call `peer.TryPopOneTimePreKeyPrivate(otkId, out var otkPrivate)`.
  - If pop fails: **throw** `InvalidOperationException` with a message that includes:
    - simulated peer id
    - requested otk id
    - count of remaining private otks on the peer (for diagnostics)

- Call X3DH:
  - Pass `localOtkPrivate: otkPrivate` when requested.
  - Pass `localOtkPrivate: null` only when no OTK requested.

**Prohibited behavior:**

- Do not silently proceed with `localOtkPrivate: null` when `request.OnetimePrekeyId` is present.
- Do not generate a new OTK or substitute a random key when missing.

#### K.3.5: Persistence (snapshot + JSON DTO) for private OTK pool

**Goal:** a restart must not break pending handshakes because private OTKs were lost.

**Files:**

- `Desktop.Wpf/Features/Simulator/PeerStateSnapshot.cs`
- `Desktop.Wpf/Features/Simulator/SimulatorState.cs` (DTOs)
- `Desktop.Wpf/Features/Simulator/JsonSimulatorStateRepository.cs`

Add a persisted representation to the peer snapshot/DTO:

- Snapshot field:
  - `IReadOnlyList<SimulatedOneTimePreKeyPrivateSnapshot>`

- DTO field:
  - `List<OneTimePreKeyPrivateDto>`

Define the DTO shape (explicit):
...
- Save:
  - `Id = record.Id.Value`
  - `PrivateKeyBytes = record.PrivateKey.Value`
  - `CreatedAtUtc = record.CreatedAtUtc`

- Load:
  - Rehydrate `PrivateKey = new PrivatePreKey(dto.PrivateKeyBytes)`
  - Rehydrate `Id = new SimulatedOneTimePreKeyId(dto.Id)`
  - Preserve timestamps

Security note (explicit): this is simulator-only state; it is acceptable for the simulator snapshot to include private key material.

#### K.3.6: Dirty tracking / persistence triggers

**File:** `Desktop.Wpf/Features/Simulator/Tracking/SimulatedPeerRuntimeTracker.cs`

Ensure the tracker subscribes to collection changes for `OneTimePreKeysPrivateMutable`.

**Rule:** any Add/Remove from the private OTK pool must mark the peer dirty (so state is persisted).

### K.4: Acceptance criteria (testable, unambiguous)

Functional:

- After relayed standard handshake, first chat message decrypts successfully (no `AuthenticationTagMismatchException`).

OTK correctness:

- When the prekey bundle response contains an OTK and main sets `HandshakeInitiatorHello.OneTimePreKeyId`:
  - the simulator responder pops that exact id from its private OTK pool
  - the responder passes it into `X3DH_Respond` as `localOtkPrivate`
  - the popped id no longer exists in `peer.OneTimePreKeysPrivate`

- When the prekey bundle response contains no OTK:
  - main does not set `OneTimePreKeyId`
  - the responder passes `localOtkPrivate: null`
  - handshake still succeeds

Persistence:

- Publish N OTKs, restart app, then accept a handshake that requests one of those ids:
  - responder finds the private key and decrypt succeeds
  - the private pool is decremented by 1 and persists

Failure behavior:

- If `request.OnetimePrekeyId` is present but the private key does not exist:
  - simulator throws (fail fast) before creating a responder session
  - error text includes simulated peer id and requested OTK id
