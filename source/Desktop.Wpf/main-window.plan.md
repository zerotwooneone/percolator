...
Make outbound sends from Main to simulated peers reliable in both cases:

- Direct (Main -> Simulated)
- Relayed (Main -> Simulated relay host -> Simulated target)

The key behavior is: any outbound send to a simulator-reserved endpoint (`127.77.*`) is *authoritatively owned by the simulator transport shim* and must be intercepted at the gRPC transport boundary.

### Constraints / Non-Goals

- Consolidate simulator interception at the transport boundary (`GrpcMessageTransportService`).
- Remove all simulator-specific relay routing logic from `MessageService`.
- Do not use `PeerId` (GUID) as an interop detection mechanism.
- Treat any literal dotted-quad `127.77.*` destination as simulator-reserved address space.
- If an outbound send targets `127.77.*` and there is no corresponding simulated peer, the send must fail fast (throw). No fallback to real network.
- **Main window persists simulated peers as normal peers** (including routing profiles / endpoints) in the main SQLite database.
- **Simulator-owned state must never be written to the main window SQLite database**, including:
...
- **Main window must not directly access simulator state**. It must communicate “as though the simulator were a real peer”, i.e., through the same transport/service seams used for real networking.
- This chunk only changes the opaque message interception API. Other interceptor methods may remain in their current shape for now.

---

### B.1 Remove simulator relay routing from MessageService

#### Intent

`MessageService` must not contain simulator-specific logic for deciding how to route messages via relay. Route planning should remain “normal” (using persisted routing profiles), and simulator behavior is implemented only by intercepting network sends at the transport boundary.

#### Changes

- `Percolator.Application/Network/MessageService.cs`
    - Remove the `_simulatorOutboundInterceptor` dependency.
    - Remove both `TryRouteMessageViaSimulatorRelayAsync(...)` call sites.

- `Percolator.Application/Network/ISimulatorOutboundInterceptor.cs`
    - Remove `TryRouteMessageViaSimulatorRelayAsync(...)`.

- `Desktop.Wpf/Features/Simulator/SimulatorOutboundInterceptor.cs`
    - Remove `TryRouteMessageViaSimulatorRelayAsync(...)` implementation.

- Tests
    - Remove/replace `Desktop.Wpf.Tests/SimulatorOutboundInterceptorRelayRoutingTests.cs` (targets the removed method).

---

### B.2 Make gRPC interception authoritative for opaque message sends

#### Intent

All outbound opaque message sends that ultimately call gRPC (`GrpcMessageTransportService`) must be intercepted when the destination endpoint is simulator-reserved (`127.77.*`). This includes:

- direct sends to a simulated peer endpoint
- relayed sends that target a simulated relay host endpoint

This interception must not require the main window to access simulator runtime state.

#### API change (locked-in)

Replace the current `TryDeliverOpaqueMessage(... out Task<DeliverOpaqueMessageResponse>)` shape with a single-call decision that can represent “not for simulator”, “delivered”, or “undeliverable”.

New types (in `Percolator.Application/Network`):

- `SimulatorOutboundInterceptResultKind`:
    - `NotForSimulator`
    - `DeliveredToSimulator`
    - `Undeliverable`

- `SimulatorOutboundInterceptResult`:
    - `Kind: SimulatorOutboundInterceptResultKind`
    - `FailureReason: string?` (optional; used when `Undeliverable`)

New method (locked-in name):

- `Task<SimulatorOutboundInterceptResult> InterceptDeliverOpaqueMessageAsync(DnsEndPoint endpoint, DeliverOpaqueMessageRequest request, CancellationToken ct)`

Update `ISimulatorOutboundInterceptor` accordingly for opaque message sends.

#### Transport behavior

`Percolator.Application/Network/GrpcMessageTransportService.cs`:

- When `_simulatorOutbound` is not null:
    - Call `InterceptDeliverOpaqueMessageAsync(...)`.
    - If `NotForSimulator`:
        - proceed with normal gRPC send.
    - If `DeliveredToSimulator`:
        - return `new DeliverOpaqueMessageResponse { Version = 1 }`.
        - do not create an HTTP client / gRPC channel.
    - If `Undeliverable`:
        - throw `InvalidOperationException` including endpoint + failure reason.
        - do not create an HTTP client / gRPC channel.

Notes:

- Do NOT introduce a new result type for `SendMessageAsync` in this chunk. Exceptions are the idiomatic transport failure mechanism here and keep call sites unchanged.

#### Simulator interceptor behavior

`Desktop.Wpf/Features/Simulator/SimulatorOutboundInterceptor.cs`:

- Detection rule:
    - Treat any destination whose `Host` is a literal dotted-quad in `127.77.*` as simulator-owned.
    - (Keep the rule explicit: this is about the destination *host string* being a literal dotted-quad, not DNS resolution.)

- Routing rule:
    - If the endpoint matches a simulated peer that is currently “listening” in the simulator runtime:
        - deliver via `_state.ReceiveOpaqueMessageFromMainAsync(...)`
        - return `DeliveredToSimulator`
    - If the endpoint is `127.77.*` but no simulated peer matches:
        - return `Undeliverable` with a useful failure reason.
    - For all non-`127.77.*` destinations:
        - return `NotForSimulator`.

Simulator delivery failures:

- Exceptions from the simulator delivery path should propagate (surface as failed send).

---

### B.3 Verification / tests

Testing guidance: follow `source/unit-testing.md` (assert public behavior/state, avoid brittle internal interaction checks).

#### Update existing Application tests

Update `Percolator.ApplicationTests/Network/SimulatorOutboundInterceptionTests.cs`:

- Add a test for `Undeliverable` asserting:
    - the send throws
    - no `IHttpClientFactory.CreateClient(...)` call occurs (external dependency; acceptable assertion)

- Update existing “intercepted” tests to reflect:
    - new interceptor method name + return type
    - `DeliveredToSimulator` returning a successful `DeliverOpaqueMessageResponse` without channel creation

#### Add/extend coverage for both message paths through the same interception boundary

- **Direct (Main -> Simulated peer):**
    - Arrange:
        - a persisted routing profile that selects a simulated peer endpoint (`127.77.*:port`).
        - simulator runtime has a simulated peer listening on that endpoint.
    - Act:
        - drive the normal send path that results in `GrpcMessageTransportService.SendMessageAsync(...)`.
    - Assert:
        - the send is intercepted (no HTTP client created)
        - and the simulator observes receipt (assert via an observable simulator-side effect exposed to tests; do not assert internal calls).

- **ViaRelay (Main -> Simulated relay host -> Simulated target):**
    - Arrange:
        - persisted routing profiles such that:
            - the relay host endpoint is `127.77.*:port`
            - the target is configured `ConnectionMode.ViaRelay` using that relay host
        - simulator runtime has the relay host simulated peer listening
    - Act:
        - drive `NetworkTransportPortAdapter.SendViaRelayAsync(...)`.
    - Assert:
        - the outbound send to the relay host is intercepted (no HTTP client created)
        - the simulator relay host has a downstream queue entry for the target PKH (assert observable queue/state, not method invocations)

- **Undeliverable (127.77.* with no simulator peer):**
    - Arrange:
        - a persisted routing profile selecting a `127.77.*:port` endpoint
        - simulator runtime does NOT have a peer listening for that endpoint
    - Act + Assert:
        - send throws
        - no gRPC channel creation occurs

### Acceptance criteria

- Direct sends (Main -> Simulated peer) work via gRPC transport interception.
- Relayed sends (Main -> Simulated relay host -> Simulated peer) result in a downstream relay queue entry in the simulator.
- Any send targeting `127.77.*` with no matching simulated peer fails the send (no fallback to network, no gRPC channel creation).
- No simulator-specific relay routing exists in `MessageService`.
- Main window continues to treat simulator peers as real peers for persistence/routing purposes.
- Simulator-owned runtime state (peers/keys/sessions/queues) is not written to the main window SQLite database, and the main window does not directly access simulator state.
