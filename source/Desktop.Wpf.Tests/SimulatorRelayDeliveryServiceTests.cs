...
public sealed class SimulatorRelayDeliveryServiceTests
{
    ...
    public async Task DeliverToPeerAsync_when_payload_is_valid_handshake_initiator_hello_upserts_pending_standard_hello_and_does_not_forward()
    {
        ...
        state.Verify(s => s.ReceiveRelayedOpaquePayloadAsync(It.IsAny<Percolator.Network.NetworkPeerId>(), It.IsAny<byte[]>(), It.IsAny<CancellationToken>()), Times.Never);
        state.Verify(s => s.EnqueueRelayUpstreamToMainAsync(It.IsAny<Percolator.Network.NetworkPeerId>(), It.IsAny<byte[]>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Never);
        state.Verify(s => s.EnqueueRelayDownstreamToPeerAsync(It.IsAny<Percolator.Network.NetworkPeerId>(), It.IsAny<Percolator.Identity.IdentityPublicKeyHash>(), It.IsAny<byte[]>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task DeliverToPeerAsync_when_initiator_pkh_matches_neither_simulated_peer_nor_main_identity_emits_routing_failure_and_does_not_enqueue()
    {
        ...
        state.Verify(s => s.UpsertPendingStandardSignalHelloAsync(It.IsAny<Percolator.Network.NetworkPeerId>(), It.IsAny<Percolator.Network.NetworkPeerId>(), It.IsAny<HandshakeInitiatorHello>(), It.IsAny<DateTimeOffset>(), It.IsAny<CancellationToken>()), Times.Never);
        state.Verify(s => s.EnqueueRelayUpstreamToMainAsync(It.IsAny<Percolator.Network.NetworkPeerId>(), It.IsAny<byte[]>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Never);
        state.Verify(s => s.EnqueueRelayDownstreamToPeerAsync(It.IsAny<Percolator.Network.NetworkPeerId>(), It.IsAny<Percolator.Identity.IdentityPublicKeyHash>(), It.IsAny<byte[]>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Never);
    }
}
