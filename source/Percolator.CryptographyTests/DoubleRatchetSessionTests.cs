...
public class DoubleRatchetSessionTests
{
    ...
    [Test]
    public void Alice_CanDecrypt_FirstRatchetMessageFromBob()
    {
        // Arrange: This test simulates the "reverse signal" flow.
        // 1. Bob gets Alice's pre-key bundle (simulated here).
        // 2. Bob acts as the Double Ratchet INITIATOR, creating the shared secret and the first message.
        // 3. Alice acts as the Double Ratchet RESPONDER, receiving this first message.
        // 4. The test ensures Alice can decrypt Bob's message.

        var logger = new NullLogger<DoubleRatchetSession>();
        var options = Options.Create(new CryptographyOptions());

        // 1. Establish keys for both parties.
        using var aliceIdentityKey = ECDiffieHellman.Create(ECCurve.NamedCurves.nistP256);
        using var alicePreKey = ECDiffieHellman.Create(ECCurve.NamedCurves.nistP256); // Alice's published pre-key.

        using var bobIdentityKey = ECDiffieHellman.Create(ECCurve.NamedCurves.nistP256);
        // This is the ephemeral key Bob uses for the X3DH calculation.
        using var bobEphemeralForX3DH = ECDiffieHellman.Create(ECCurve.NamedCurves.nistP256);

        // 2. Bob calculates the shared secret using Alice's keys.
        // In a real X3DH, this would be more complex, but for the ratchet test, a simple derivation is sufficient.
        var sharedSecret = new SharedSecret(bobIdentityKey.DeriveKeyMaterial(aliceIdentityKey.PublicKey));

        // 3. Bob, as the INITIATOR, creates his session.
        // He generates a new ephemeral key specifically for his first ratchet message.
        using var bobRatchetEphemeral = ECDiffieHellman.Create(ECCurve.NamedCurves.nistP256);

        using var bobSession = DoubleRatchetSession.AsInitiator(
            sharedSecret,
            new RatchetIdentityKey(aliceIdentityKey.PublicKey.ExportSubjectPublicKeyInfo()), // Alice's ID
            new RatchetEphemeralKey(alicePreKey.PublicKey.ExportSubjectPublicKeyInfo()), // Alice's PreKey
            bobRatchetEphemeral, // Bob's first ratchet key
            logger,
            options);

        // 4. Bob encrypts the first message of the session.
        var bobPlaintext = new Plaintext(Encoding.UTF8.GetBytes("Session-ID-is-XYZ"));
        var firstMessageFromBob = bobSession.Encrypt(bobPlaintext);
        var header = firstMessageFromBob.GetHeader();

        // 5. Alice, as the RESPONDER, creates her session to receive the message.
        using var aliceSession = DoubleRatchetSession.AsResponder(
            sharedSecret,
            new RatchetIdentityKey(bobIdentityKey.PublicKey.ExportSubjectPublicKeyInfo()), // Bob's ID
            header.PreKey, // The ephemeral key from Bob's message header
            alicePreKey, // Alice uses her pre-key that Bob used to initiate.
            logger,
            options);

        // Act
        var decryptedByAlice = aliceSession.Decrypt(firstMessageFromBob);

        // Assert
        Encoding.UTF8.GetString(decryptedByAlice.Value).Should().Be("Session-ID-is-XYZ");
    }

    [Test]
    public void Full_ReverseSignal_Lifecycle_With_Serialization()
    {
        // Arrange: This test simulates the complete, real application flow.
        var logger = new NullLogger<DoubleRatchetSession>();
        var options = Options.Create(new CryptographyOptions());

        // 1. Keys are established for Alice (responder) and Bob (initiator).
        using var aliceIdentity = ECDiffieHellman.Create(ECCurve.NamedCurves.nistP256);
        using var alicePreKey = ECDiffieHellman.Create(ECCurve.NamedCurves.nistP256);
        using var bobIdentity = ECDiffieHellman.Create(ECCurve.NamedCurves.nistP256);
        using var bobRatchetEphemeral = ECDiffieHellman.Create(ECCurve.NamedCurves.nistP256);
        var bobRatchetEphemeralKey = new RatchetEphemeralKey(bobRatchetEphemeral.PublicKey.ExportSubjectPublicKeyInfo());
        var sharedSecret = new SharedSecret(bobIdentity.DeriveKeyMaterial(aliceIdentity.PublicKey));

        // 2. Bob (Ratchet Initiator) creates the first message.
        using var bobSession = DoubleRatchetSession.AsInitiator(
            sharedSecret,
            new RatchetIdentityKey(aliceIdentity.PublicKey.ExportSubjectPublicKeyInfo()),
            new RatchetEphemeralKey(alicePreKey.PublicKey.ExportSubjectPublicKeyInfo()),
            bobRatchetEphemeral,
            logger, options);
        var firstMessageFromBob = bobSession.Encrypt(new Plaintext("Session-ID"u8.ToArray()));

        // 3. Alice (Ratchet Responder) receives the message.
        
        using var initialAliceSession = DoubleRatchetSession.AsResponder(
            sharedSecret,
            new RatchetIdentityKey(bobIdentity.PublicKey.ExportSubjectPublicKeyInfo()),
            bobRatchetEphemeralKey, //this also works : firstMessageFromBob.GetHeader().PreKey,
            alicePreKey,
            logger, options);
        initialAliceSession.Decrypt(firstMessageFromBob);

        // 4. CRITICAL: Alice's state is saved and reloaded into a new session object.
        var aliceState = initialAliceSession.GetState();
        // Without the fix, aliceState.RatchetFlag is false here. With the fix, it is true.
        using var reloadedAliceSession = new DoubleRatchetSession(aliceState, logger, options);

        // 5. The reloaded Alice session sends a reply. This triggers her first sending ratchet.
        var replyFromAlice = reloadedAliceSession.Encrypt(new Plaintext(Encoding.UTF8.GetBytes("ping")));

        // 6. Bob's state is saved and reloaded.
        var bobState = bobSession.GetState();
        using var reloadedBobSession = new DoubleRatchetSession(bobState, logger, options);

        // Act
        // 7. The reloaded Bob session decrypts Alice's reply.
        // This is where the original bug occurred. Bob would generate a bad key.
        var decryptedByBob = reloadedBobSession.Decrypt(replyFromAlice);

        // Assert
        Encoding.UTF8.GetString(decryptedByBob.Value).Should().Be("ping");

        // 8. Go one step further: Bob replies again.
        var finalReplyFromBob = reloadedBobSession.Encrypt(new Plaintext(Encoding.UTF8.GetBytes("pong")));
        var finalDecryptedByAlice = reloadedAliceSession.Decrypt(finalReplyFromBob);
        Encoding.UTF8.GetString(finalDecryptedByAlice.Value).Should().Be("pong");
    }
}