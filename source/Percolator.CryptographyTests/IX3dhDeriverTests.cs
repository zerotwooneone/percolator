...
public class IX3dhDeriverTests
{
    ...
    private static OneTimeKey NewOneTimePreKey() {
        using var ecdh = ECDiffieHellman.Create(ECCurve.NamedCurves.nistP256);
        return OneTimeKey.FromBytes(ecdh.PublicKey.ExportSubjectPublicKeyInfo());
    }
    ...
    public void Initiator_and_Responder_match_IRK_and_ephemeral_is_returned()
    {
        ...
        Assert.That(init.InitialRootKey.ToArray().Length, Is.EqualTo(32));
        Assert.That(init.InitiatorEphemeralPublicKey.ToArray().Length, Is.GreaterThan(0));

        var resp = deriver.DeriveResponder(initiatorIdentity.pub, init.InitiatorEphemeralPublicKey, remoteIdPriv, spkPriv, null);
        Assert.That(resp.InitialRootKey.ToArray(), Is.EqualTo(init.InitialRootKey.ToArray()));
    }
}
