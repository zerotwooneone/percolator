...
- **R8 (Complete): Schema update and indexing**
  - Dropped `InitiatorEphemeralPrivateKey` from prehandshake schema.
  - Added `RemoteIdentityKeySpkiHash` and composite index `(SelfIdentityId, RemoteIdentityKeySpkiHash, CreatedAtUtc)`.

- **R3: Implement Initiator Finalize**
  - Implement `DirectSessionManager.FinalizeAsInitiatorAsync` to initialize Double Ratchet from IRK and responder’s header ratchet key.
  - No initiator ephemeral private key is required or persisted.

- **R4: Wire ResponderHello handler finalize path**
  - In `HandleHandshakeResponderHelloHandler` slow-path:
    - Resolve prehandshake by `(SelfIdentityId, RemoteIdentityKeySpkiHash)` ordered by recency.
    - Call `FinalizeAsInitiatorAsync` with IRK and responder’s `public_ratchet_key`.
    - Delete the prehandshake record on success.

- **R5: Responder establishment alignment**
  - Ensure responder assigns `session_id` and establishes immediately on initiator hello handling.
  - Emit first ratchet message whose header carries the responder’s next `public_ratchet_key`.
  - Persist session and upsert header key index for fast-path resolution.

- **R7: Tests**
  - Add parity tests for fast-path header resolution and slow-path finalize parsing of `direct_session_id`.
  - Add tests for secure messaging boundary (decrypt after finalize; no DSM encrypt/decrypt usage).

- **R2 (Cleanup): Remove legacy initiator establish API**
  - After R3/R4/R5 and tests are green, remove legacy `EstablishSessionAsInitiatorAsync` overloads and any `CompleteHandshakeAsync` shims.
  - Fix all call sites to use finalize path.

### Reverse-Signal (RS) Remediation (Updated)

- **RS1: Invitation persistence & privacy**
  - Persist only required invitation metadata and public keys. If any private key material is temporarily required, encrypt at rest and enforce strict TTL with purge job and tests.

- **RS2: Role inversion correctness**
  - Acceptor (invitee) performs X3DH as initiator and sends the first DR message with correct header `public_ratchet_key`.
  - Responder (inviter) establishes immediately, assigns `session_id`, and persists+indexes header key.

- **RS3: Inviter finalize path**
  - Inviter finalizes using IRK and the acceptor’s header `public_ratchet_key` from the first message; decrypt inner payload to read `direct_session_id`.
  - Use `FinalizeAsInitiatorAsync`; remove invitation record on success.

- **RS4: Protobuf presence/versioning**
  - Ensure invite request/response messages carry top-level `version` and optional fields; encrypted inner payload must include `direct_session_id` where applicable.
