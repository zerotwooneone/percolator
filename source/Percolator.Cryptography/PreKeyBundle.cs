...
/// </summary>
public record PreKeyBundleForPublish
{
    /// <summary>
    ...
    public IReadOnlyCollection<OneTimeKeyInstance> OneTimeKeys { get; }

    public PreKeyBundleForPublish(
        RatchetIdentityKey identitySigningKey,
        Guid signedPreKeyId,
        PreKey signedPreKey,
        Signature signedPreKeySignature,
        IEnumerable<OneTimeKeyInstance>? oneTimeKeys,
        DateTimeOffset? expirationDateUtc = null)
    {
        ...
        OneTimeKeys = (oneTimeKeys as OneTimeKeyInstance[] ?? oneTimeKeys?.ToArray() ) ?? Array.Empty<OneTimeKeyInstance>();
...
/// Once used, the key is discarded by the server. This provides strong forward secrecy and deniability.
/// </summary>
public sealed record OneTimeKeyInstance(Guid Id, OneTimeKey Key);
