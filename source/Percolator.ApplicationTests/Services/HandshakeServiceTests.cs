...
public class HandshakeServiceTests
{
    ...
    public async Task InitiateStandardHandshake_returns_session_and_initial_cipher_when_plaintext_provided()
    {
        ...
        var result = await svc.InitiateStandardHandshakeAsync(new CryptoPeerId(1), Plaintext.FromBytes(new byte[]{0xAA}), CancellationToken.None);
    ...
    public async Task InitiateStandardHandshake_initial_cipher_decrypts_on_complementary_responder()
    {
        ...
        var compose = await svc.InitiateStandardHandshakeAsync(new CryptoPeerId(2), plaintext, CancellationToken.None);
