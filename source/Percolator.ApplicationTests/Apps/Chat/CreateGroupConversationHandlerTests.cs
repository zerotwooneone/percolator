...
namespace Percolator.ApplicationTests.Apps.Chat
{
    ...
    public sealed class CreateGroupConversationHandlerTests
    {
        ...
        public async Task CreateGroupConversation_CreatesGroup_And_SeedsCreatorAdminKey()
        {
            ...
            var aliceIdentityPublicKeyHash = IdentityPublicKeyHash.FromBytes(alicePkh);
            var bobIdentityPublicKeyHash = IdentityPublicKeyHash.FromBytes(bobPkh);
            var charlieIdentityPublicKeyHash = IdentityPublicKeyHash.FromBytes(charliePkh);

            var alicePeer = new PeerId(Guid.NewGuid());
            var bobPeer = new PeerId(Guid.NewGuid());
            var charliePeer = new PeerId(Guid.NewGuid());

            keyStore.Setup(s => s.GetPeerIdByPublicKeyHashAsync(It.Is<IdentityPublicKeyHash>(h => h.Equals(aliceIdentityPublicKeyHash)), It.IsAny<CancellationToken>()))
                .ReturnsAsync(alicePeer);
            keyStore.Setup(s => s.GetPeerIdByPublicKeyHashAsync(It.Is<IdentityPublicKeyHash>(h => h.Equals(bobIdentityPublicKeyHash)), It.IsAny<CancellationToken>()))
                .ReturnsAsync(bobPeer);
            keyStore.Setup(s => s.GetPeerIdByPublicKeyHashAsync(It.Is<IdentityPublicKeyHash>(h => h.Equals(charlieIdentityPublicKeyHash)), It.IsAny<CancellationToken>()))
