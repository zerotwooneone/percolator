...
public class RelayOrchestratorTests
{
    ...
    private static (RelayOrchestrator orchestrator,
        ...
        Mock<IMessageTransportService> transport) Create(out ActiveIdentityContext active)
    {
        ...
        active = Active();
        var orchestrator = new RelayOrchestrator(logger, queue.Object, directSessions.Object, sessionMgr.Object, transport.Object, active);
        return (orchestrator, queue, directSessions, sessionMgr, transport);
    }

    [Test]
    public async Task RelayNextAsync_happy_ack_deletes_and_returns_true()
    {
        var (orchestrator, queue, directSessions, sessionMgr, transport) = Create(out var active);
        ...
        sessionMgr.Setup(s => s.EncryptMessageAsync(sessionId, It.IsAny<Plaintext>()))
        ...
        // DR decrypt of ack payload yields RelayOpaqueResponse with same ack id
        var ack = new RelayOpaqueResponse { Version = 1, MessageAckId = ByteString.CopyFrom(ackId.ToByteArray()) };
        sessionMgr.Setup(s => s.ReceiveMessageAsync(sessionId, It.IsAny<SessionRatchetMessage>()))
            .ReturnsAsync(new Plaintext(ack.ToByteArray()));
    ...
    public void RelayNextAsync_ack_mismatch_throws()
    {
        ...
        sessionMgr.Setup(s => s.ReceiveMessageAsync(sessionId, It.IsAny<SessionRatchetMessage>()))
            .ReturnsAsync(new Plaintext(mismatched.ToByteArray()));
    ...
    public void RelayNextAsync_no_response_payload_throws()
    {
        ...
        directSessions.Setup(d => d.GetByRemotePeerIdAsync(networkPeerId, active.Identity!.SelfIdentityId))
            .ReturnsAsync(new DirectSession(networkPeerId, directSessionId));
        sessionMgr.Setup(s => s.EncryptMessageAsync(It.IsAny<SessionId>(), It.IsAny<Plaintext>()))
            .ReturnsAsync(new SessionRatchetMessage(new byte[] { 0xAA }));
