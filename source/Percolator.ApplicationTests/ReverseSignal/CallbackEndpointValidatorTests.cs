...
public sealed class CallbackEndpointValidatorTests
{
    ...
    {
        var v = new CallbackEndpointValidator(Options.Create(new ReverseSignalOptions { AllowLan = false }));
    ...
    {
        var v = new CallbackEndpointValidator(Options.Create(new ReverseSignalOptions { AllowLan = true }));
        var result = v.Validate("127.0.0.1", 443);
        result.IsValid.Should().BeTrue();
        result.IsIpAddress.Should().BeTrue();
        result.IsLanTarget.Should().BeTrue();
